...
type Columnist = { id: string; name: string; avatar: string; bio: string; professionalInfo: string; columnistSlug: string; socialLinks: Array<{ label: string; url: string }> };
...
export default function ColumnistProfilePage() {
  ...
  return <main className="mx-auto max-w-5xl px-4 py-12"><section className="rounded-2xl bg-gray-900 p-8 text-white md:flex md:items-center md:gap-8"><img src={columnist.avatar || '/logo-oficial.png'} alt={columnist.name} className="h-32 w-32 rounded-full object-cover" /><div><p className="text-sm font-semibold uppercase tracking-widest text-red-300">Colunista RBN</p><h1 className="mt-2 text-4xl font-bold">{columnist.name}</h1><p className="mt-3 max-w-2xl text-gray-200">{columnist.bio}</p><p className="mt-2 text-sm text-gray-300">{columnist.professionalInfo}</p></div></section><h2 className="mt-10 text-2xl font-bold">Matérias de {columnist.name}</h2><div className="mt-5 grid gap-4 md:grid-cols-2">{authored.map((article) => <Link key={article.id} href={`/artigo/${article.id}`} className="rounded-xl border bg-white p-5 shadow-sm hover:border-red-300"><h3 className="font-bold">{article.title}</h3><p className="mt-2 text-sm text-gray-500">{article.excerpt}</p></Link>)}</div></main>;
}
