...
export default function TeamDocumentationPage() {
  ...
  const openForm = (mode: 'upload' | 'request', userId = selectedUserId || (users[0]?.id ?? currentUser?.id ?? '')) => {
    setFormMode(mode);
    setForm((current) => ({ ...current, userId: userId || (users[0]?.id ?? currentUser?.id ?? ''), documentType: '', selectedTypes: [], file: null }));
    setShowForm(true);
  };
