...
export async function saveStoredComment(id: string, payload: Record<string, unknown>) {
  ...
  const fallback = await fetch(fallbackTable, { method: 'POST', headers: { ...headers(), Prefer: 'resolution=merge-duplicates,return=minimal' }, body: JSON.stringify({ id: `comment:${id}`, payload, updated_at: new Date().toISOString() }), cache: 'no-store' });
...
export async function deleteStoredComment(id: string) {
  ...
  const fallback = await fetch(`${fallbackTable}?id=eq.${encodeURIComponent(`comment:${id}`)}`, { method: 'DELETE', headers: { ...headers(), Prefer: 'return=minimal' }, cache: 'no-store' });
